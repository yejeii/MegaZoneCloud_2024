...
public class SingleLinkedList<E> {
	...
	public void addFirst(E value) {
		...
		// 3. 요소가 추가되었으니 size 를 늘린다.
		size++;

		// 4. 맨앞에 요소가 추가되었으니 head 를 업데이터 한다.
		head = newNode;

		// 5. 만일 최초로 요소가 add 된 것이면 head 와 tail 이 가리키는 요소는 같게 한다.
		if (head == tail)
			tail = newNode;
	}
	...
	public void addLast(E value) {
		...
		// 2. 새 노드 생성 (맨 마지막 요소 추가니까 next 는 null)
		Node<E> newNode = new Node<>(value, null);

		// 3. 요소가 추가되었으니 size 를 늘린다.
		size++;

		// 4. 맨 뒤에 요소가 추가 되었으니 tail 을 업데이트
		tail = newNode;
		if (last == null) {
			// 5. 만일 최초로 요소가 add 된 것이면 head 와 tail 이 가리키는 요소는 같게 된다.
			head = newNode;
		} else {
			// 6. 최초 추가가 아니라면 last 변수 (추가 되기 전 마지막이었던 요소)에서 추가 된 새 노드를 가리키게 됨.
			last.next = newNode;
		}
	}
	...
	public static void main(String[] args) {
		...
		l.remove(3);
		l.remove(3);
		System.out.println(l); // [3, 4, 3, 11, 100]

		System.out.println(l.get(4)); // 100

		l.set(4, 999);
		System.out.println(l); // [3, 4, 3, 11, 999]
	}
}
